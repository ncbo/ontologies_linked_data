...
module LinkedData
  ...
  GROUP BY ?ontology
    ...
    def self.create_mapping_count_totals_for_ontologies(logger, arr_acronyms)
      ...
      persistent_counts = self.all_existent_mapping_counts(pair_count: false)
      latest = retrieve_latest_submissions
      self.delete_zombie_submission_count(persistent_counts, latest)
      ...
      new_counts.each_key do |acr|
        ...
        ctr += 1
        self.update_mapping_count(persistent_counts, new_counts, acr, acr, new_count, false)
    ...
    def self.create_mapping_count_pairs_for_ontologies(logger, arr_acronyms)
      ...
      persistent_counts = self.all_existent_mapping_counts(pair_count: true)
      ...
      latest_submissions.each do |acr, sub|
        ...
        persistent_counts = self.all_existent_mapping_counts(acr: acr, pair_count: true)
        self.delete_zombie_mapping_count(persistent_counts, new_counts)
        ...
        new_counts.each_key do |other|
          ...
          ctr += 1
          self.update_mapping_count(persistent_counts, new_counts, acr, other, new_count, true)
          remaining = num_counts - ctr
      ...
      return <<-eos
{
  ...
  #{bind}
    ...
    def self.calculate_and_log_counts(uri, logger, reload_cache, label, enable_debug)
      start_time = Time.now
    ...
    def self.update_mapping_count(persistent_counts, new_counts, acr, other, new_count, pair_count)
    ...
    def self.delete_zombie_mapping_count(persistent_counts, new_counts)
    ...
    def self.delete_zombie_submission_count(persistent_counts, all_latest_submissions)
    ...
    def self.all_existent_mapping_counts(acr: nil, pair_count: true)
      persistent_counts = {}
